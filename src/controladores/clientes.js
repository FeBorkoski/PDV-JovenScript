...
const editarCliente = async (req, res) => {
    ...
    try {
        ...
        const cadastroExistente = await knex("clientes").where(function () {
            this.where({ email }).orWhere({ cpf })
        }).andWhere('id', '!=', id).first();

        if (cadastroExistente) {
            return res.status(400).json({ mensagem: "E-mail e/ou cpf já cadastrados" });
        }

        await knex("clientes").update({
            nome, email: emailFormatado, cpf, cep, rua, numero, bairro, cidade, estado
        }).where({ id });
